...
class ApiClient extends GetxService {
  ...
  ApiClient({required this.appBaseUrl, required this.sharedPreferences}) {
    ...
    List<int> ? zoneIds;
    try {
      String? userAddress = sharedPreferences.getString(AppConstants.USER_ADDRESS);
      if(userAddress != null){
    ...
    if(GetPlatform.isWeb && sharedPreferences.containsKey(AppConstants.MODULE_ID)) {
      String? moduleData = sharedPreferences.getString(AppConstants.MODULE_ID);
      try {
        if(moduleData != null){
          moduleID = ModuleModel.fromJson(jsonDecode(moduleData)).id;
        }
    ...
    updateHeader(token, zoneIds, languageCode, moduleID);
  }
  ...
  void updateHeader(String? token, List<int>? zoneIDs, String? languageCode, int? moduleID) {
    ...
    if(zoneIDs != null && zoneIDs.isNotEmpty) {
      ...
      _header [AppConstants.MODULE_ID] = jsonEncode(zoneIDs);
    }

    if(moduleID != null) {
      _header[AppConstants.MODULE_ID] = moduleID.toString();
      //_header.addAll({AppConstants.MODULE_ID: moduleID.toString()});
    }
    _header.removeWhere((key, value) => value.isEmpty);
